...
class RaidAutomator(ClusterSetup):
	...
	def run(self, nodes, master, user, user_shell, volumes):
		...
		for i in range(int(vol_num)):
			...
			vol = check_output(["aws", "ec2", "create-volume", "--size", str(vol_size), "--region", "us-east-1", "--availability-zone", availability_zone, "--volume-type", "gp2"])
		...
		time.sleep(10)
		master.ssh.execute("sudo mdadm --create --verbose /dev/md0 --level=0 --name=my_raid --raid-devices=%s %s" %(vol_num,all_dev))
