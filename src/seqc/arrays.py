...
class JaggedArray:
    ...
    # todo check if this is working; how are features stored?
    def is_unique(self):
        """
...
class ReadArray:
    ...
    def from_samfile(cls, samfile, feature_converter):
        ...
        with open(samfile, 'r') as f:
            ...
            for i, s in enumerate(ma_sizes):
                ...
                if feat:
                    length = len(feat)
                    features[current_index_position:current_index_position + length] = feat
                    positions[current_index_position:current_index_position + length] = pos
                    index[i, :] = (current_index_position, current_index_position + length)
                    current_index_position += length
                else:
                    index[i, :] = (current_index_position, current_index_position)
    ...
    def _process_multialignment(cls, alignments, feature_converter):
        ...
        if len(alignments) == 1:
            ...
            if flag & 4:
                ...
                pos = int(first[3])
                strand = b'-' if (flag & 16) else b'+'
                reference_name = first[2].encode()
                assert isinstance(reference_name, bytes)
            ...
            for alignment in alignments:
                ...
                strand = b'-' if (flag & 16) else b'+'
                reference_name = alignment[2].encode()
                assert isinstance(reference_name, bytes)
                assert isinstance(pos, int)
    ...
    def mask_failing_records(self, n_poly_t_required):
        ...
        if self._failing_mask is not None:
            return self._failing_mask
        ...
        self._failing_mask = vbool & has_feature
        return self._failing_mask
    ...
    def save_h5(self, archive_name):
        ...
        if len(self.features.data) == 0:
            raise ValueError('cannot store empty positions array. There is likely '
                             'something wrong with ReadArray construction or feature '
                             'translation')
        feature_group = seqc.h5.create_group(
            f, 'features', '/', 'Data for ReadArray._features')
        store_carray(f, self.features.data, feature_group, 'data')
        store_carray(f, self.features.index, feature_group, 'index')

        # store position data
        if len(self.positions.data) == 0:
            raise ValueError('cannot store empty positions array. There is likely '
                             'something wrong with ReadArray construction or feature '
                             'translation')
        positions_group = seqc.h5.create_group(
            f, 'positions', '/', 'Data for ReadArray._positions')
        store_carray(f, self.positions.data, positions_group, 'data')
        store_carray(f, self.positions.index, positions_group, 'index')
    ...
    def unique_features_to_sparse_counts_cmp(self, collapse_molecules, n_poly_t_required,
                                             support_required=2):
        ...
        if collapse_molecules:
            ...
            for i in unmasked_inds:
                feature = self.data['unique_features'][i]
                # if len(feature) > 1:
                #     continue
                cell = self.data['cell'][i]
                rmt = self.data['rmt'][i]
    ...
    def _sort_masked(self, min_poly_t):
        """
        Lexicographical argsort (indirect) of passing records in self. Will create a mask
        of records failing filters if this has not been already carried out.
        ...
        """
        if self._failing_mask is None:
            mask = self.mask_failing_records(min_poly_t)
        else:
            mask = self._failing_mask
        self._sorted = np.lexsort((
            self.data['rmt'][mask],
            self.data['unique_features'][mask],
            self.data['cell'][mask]))

    # todo | reminder | will only work if disambiguation has been run.
    def to_sparse_counts_diff(self, required_support=0, min_poly_t=3):
        ...
        if self._sorted is None:
            self._sort_masked(min_poly_t)

        sort_ord = self._sorted
        data = self.data[self._failing_mask]

        # find boundaries between cell, rmt, and feature
        all_diff = np.zeros(data.shape[0], dtype=np.bool)
        all_diff[0] = True  # keep the first one; it's different from void (preceding)
        all_diff[1:] |= np.diff(data['cell'][sort_ord]).astype(np.bool)  # diff cell
        all_diff[1:] |= np.diff(data['rmt'][sort_ord]).astype(np.bool)  # diff rmt
        all_diff[1:] |= np.diff(data['unique_features'][sort_ord]).astype(np.bool)  # diff feature
        ...
        # calculations
        i = np.concatenate((np.where(all_diff)[0], [data.shape[0]]))
        rpm_count = np.diff(i)
        ...
        rpc_diff = np.zeros(data.shape[0], dtype=np.bool)
        rpc_diff[0] = True
        rpc_diff[1:] |= np.diff(data['cell'][sort_ord]).astype(np.bool)
        rpc_diff[1:] |= np.diff(data['unique_features'][sort_ord]).astype(np.bool)
        i = np.concatenate((np.where(rpc_diff)[0], [data.shape[0]]))
        ...
        mpc_diff[0] = True
        mpc_diff[1:] |= np.diff(data['cell'][ra_molecule_idx]).astype(np.bool)
        mpc_diff[1:] |= np.diff(data['unique_features'][ra_molecule_idx]).astype(np.bool)
        ...
        # reads per cell
        row, rcells = map_to_unique_index(data['cell'][ra_rpc_idx])
        col, rgenes = map_to_unique_index(data['unique_features'][ra_rpc_idx])
        shape = (len(rcells), len(rgenes))
        rpc = coo_matrix((rpc_count, (row, col)), shape=shape, dtype=np.int32)
        # reads_per_cell = seqc.analyze.SparseCounts(rpc, rcells, rgenes)

        # molecules per cell
        row, mcells = map_to_unique_index(data['cell'][ra_rpm_idx])
        col, mgenes = map_to_unique_index(data['unique_features'][ra_rpm_idx])
        shape = (len(mcells), len(mgenes))
        mpc = coo_matrix((mpc_count, (row, col)), shape=shape, dtype=np.int32)
        # molecules_per_cell = seqc.analyze.SparseCounts(mpc, mcells, mgenes)

        # self._experiment = seqc.analyze.Experiment(reads_per_cell, molecules_per_cell)
        # return self._experiment

        # todo this is the old style instead of the SparseCounts and Experiments objects
        return {'reads': {'matrix': rpc, 'row_inds': rcells, 'col_inds': rgenes},
                'molecules': {'matrix': mpc, 'row_inds': mcells, 'col_inds': mgenes}}
...
class FilteredArray:
    ...
    def __init__(self, data):
        """
        :param data:  filtered np.recarray or structured array
        :return:  self
        """
        self._data = data
    ...
    @classmethod
    def from_read_array(cls, read_array: ReadArray, min_poly_t: int):
        """Return a filtered array from a ReadArray structure
        ...
        mask = (
            (read_array.data['cell'] != 0) &
            (read_array.data['rmt'] != 0) &
            (read_array.data['n_poly_t'] >= min_poly_t) &
            (read_array.features.is_unique()))

        data = read_array.data[mask]
        features = read_array.features.to_unique()
        positions = read_array.positions.to_unique()

        # todo remove me
        assert len(features) == len(data) == len(positions)
        data = append_fields(data, ['features', 'positions'], [features, positions])
        return cls(data)

    def _indirect_sort(self):
        return np.lexsort((
            self.data['rmt'],
            self.data['features'],
            self.data['cell'],))

    def to_expression_matrices(self, return_reads=True):
        """return an expression matrix of cells x genes whose entries are molecule counts

        :param return_reads:  if True, also returns a matrix of read counts
        :return: expression matrix
        """

        # get sorted index

        sort_ord = self._indirect_sort()

        # find boundaries between cell, rmt, and feature
        all_diff = np.zeros(self.data.shape[0], dtype=np.bool)
        all_diff[0] = True  # keep the first one; it's different from void (preceding)
        ...
        all_diff[1:] |= np.diff(self.data['features'][sort_ord]).astype(np.bool)  # diff feature

        # get key_index (into original ReadArray) and reads per molecule counts
        # adding required support here will cut any molecules with < required_support from
        # downstream steps that rely upon ra_molecule_idx (such as reads per molecule
        # calculations
        i = np.concatenate((np.where(all_diff)[0], [self.data.shape[0]]))
        rpm_count = np.diff(i)
        ...
        # on only cell and feature from the original sort
        # diff is working as intended
        rpc_diff = np.zeros(self.data.shape[0], dtype=np.bool)
        rpc_diff[0] = True
        rpc_diff[1:] |= np.diff(self.data['cell'][sort_ord]).astype(np.bool)
        rpc_diff[1:] |= np.diff(self.data['features'][sort_ord]).astype(np.bool)
        i = np.concatenate((np.where(rpc_diff)[0], [self.data.shape[0]]))
        ra_rpc_idx = sort_ord[i[:-1]]
        rpc_count = np.ravel(np.diff(i))
        ...
        mpc_diff[1:] |= np.diff(self.data['features'][ra_molecule_idx]).astype(np.bool)
        i = np.concatenate((np.where(mpc_diff)[0], [len(ra_molecule_idx)]))
        ra_rpm_idx = ra_molecule_idx[i[:-1]]
        mpc_count = np.ravel(np.diff(i))
        ...
        # molecules per cell
        row, mcells = map_to_unique_index(self.data['cell'][ra_rpm_idx])
        col, mgenes = map_to_unique_index(self.data['features'][ra_rpm_idx])
        shape = (len(mcells), len(mgenes))
        mpc = coo_matrix((mpc_count, (row, col)), shape=shape, dtype=np.int32)
        # molecules_per_cell = seqc.analyze.SparseCounts(mpc, mcells, mgenes)
        ...
        # reads per cell
        if return_reads:
            row, rcells = map_to_unique_index(self.data['cell'][ra_rpc_idx])
            col, rgenes = map_to_unique_index(self.data['features'][ra_rpc_idx])
            shape = (len(rcells), len(rgenes))
            rpc = coo_matrix((rpc_count, (row, col)), shape=shape, dtype=np.int32)
            # reads_per_cell = seqc.analyze.SparseCounts(rpc, rcells, rgenes)

            output = {'reads': {'matrix': rpc, 'row_ids': rcells, 'col_ids': rgenes},
                      'molecules': {'matrix': mpc, 'row_ids': mcells, 'col_ids': mgenes}}

        else:
            output = {'molecules': {'matrix': mpc, 'row_ids': mcells, 'col_ids': mgenes}}
