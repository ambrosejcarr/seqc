...
# todo | there should be a check on empty jagged array data when attempting to store ReadArray as h5; throws errors otherwise
class JaggedArray:
...
class SimpleReadArray:
    ...
    def from_samfile(cls, samfile, feature_converter):
        ...
        with open(samfile, 'r') as f:
            records = iter(f)
            list(islice(records, header_size))  # get rid of header
            for i, s in enumerate(ma_sizes):
                # get all records associated with a multialignment
                multi_alignment = list(islice(records, s))
                if not multi_alignment:  # iterator is exhausted
        ...
        # positions = positions[:current_index_position]
        #
        jagged_features = JaggedArray.from_iterable(features)
        jagged_positions = JaggedArray.from_iterable(positions)
        #
        return cls(read_data, jagged_features, jagged_positions)
    ...
    def _process_multialignment(cls, alignments, feature_converter):
        ...
        # parse data from name field, previously extracted from forward read
        cell, rmt, n_poly_t, valid_cell, trimmed_bases, fwd_quality = (
            int(f) for f in first[0].strip().split(';')[0].split(':'))
        ...
        if len(alignments) == 1:
            ...
            for alignment in alignments:
                ...
                features.extend(feature_converter.translate(strand, reference_name, pos))
                positions.append(pos)
