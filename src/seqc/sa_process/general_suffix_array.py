...
def binary_search(l, r, x, target):
    """
    Goal is to find the number in the list that is closest to the query but less than
    or equal to it. Will return -1 if no such number exists in the list
    (query < min(list)).
    """
    if r < l:
        return -1
...
class GeneralSuffixArray:
    ...
    def __init__(self,
                 file_name="/home/tb/dpeer_work/cterm_mappability/transcripts_with_unique_ends.fa",
                 test_limited=False):
        ...
        genes = seqc.sa_process.load_fasta.load_fasta(in_read)
        ...
        print(
            "Done constructing object. Now you can explicitly build the SA and the LCP array and such.")
    ...
    def transform_input(self, code=None):
       ...
       flat list."""
        ...
        gene_ends = [cc[len(word) + 1] for word in
                     self.words]  #by virtue of custom ConvenienceCounter class, this keeps track of ends... they're actually 1-based ends, though from what I can tell.
        gene_ends = [x - 1 for x in
                     gene_ends]  #to make it zero-based which is probably preferable.
        translated = [item for sublist in
                      [[code[i] for i in self.words[x]] + [(starter + x)] for x in
                       range(num_words)] for item in sublist]
        #print translated
        semi_translated = "".join(
            [item for sublist in [[i for i in x] + ["/"] for x in self.words] for item in
             sublist])
    ...
    def construct_SA(self):
        ...
        start = time.time()
        self.sa = seqc.sa_process.basic_suffix_array.suffix_array(
            self.transput + [0, 0, 0], [0] * self.total_len, self.total_len,
            len(self.final_code) + len(self.words) + 1)
    ...
    def construct_lcp(self):
        ...
        if self.sa is not None and self.transput is not None:
            self.lcp_tab = seqc.sa_process.lcp.get_lcp(self.sa, self.transput)
            print(time.time() - start)
    ...
    def esa_up_down(self):
        ...
        for i in range(1, n):
            while self.lcp_tab[i] < self.lcp_tab[stack[-1]]:
                ...
                if self.lcp_tab[i] <= self.lcp_tab[stack[-1]] and self.lcp_tab[
                    stack[-1]] != self.lcp_tab[last_index]:
    ...
    def esa_find(self, pattern):
        ...
        while i is not None and j is not None and c < len_pat and query_found:
            if i != j:
                ...
                query_found = self.transput[
                              self.sa[i] + c: self.sa[i] + mini - 1] == pattern[
                                                                        c:mini - 1]
                ...
                query_found = self.transput[
                              self.sa[i] + c: self.sa[i] + len_pat - 1] == pattern[
                                                                           c:len_pat - 1]  #it would be helpful to understand how to make this else condition work properly
                #print query_found
    ...
    def deal_with_patterns(self, min_k=50, output_dir="/home/tb/misc_output/",
                           jelly_bin="/home/tb/dpeer_work/jellyfish-2.2.0/bin/jellyfish",
                           jelly_processes=16, jelly_s_str="400M"):
        end_list = self.gene_ends
        gene_list = self.gene_names
        end_len = len(end_list)
        all_patterns = []
        k = min_k
        sorted_indices = sorted(list(range(len(self.lcp_tab))),
                                key=lambda indx: -self.lcp_tab[indx])
        patterns = []
        ...
        for biggest_overlap in sorted_indices:
            if self.lcp_tab[biggest_overlap] < k:
                break
            pattern = tuple(self.transput[
                            self.sa[biggest_overlap]:self.sa[biggest_overlap] +
                                                     self.lcp_tab[biggest_overlap]])
            str_pat = "".join(
                ['?ACGTN'[x] for x in pattern])  #str_pat = 1/2 memory of the native list
        ...
        command = jelly_bin + " count -m " + str(
            k) + " -o /dev/stdout /dev/stdin -t " + str(
            jelly_processes) + " -s " + jelly_s_str
        jelly_process = subprocess.Popen(command.split(), stdin=subprocess.PIPE,
        ...
        dump_process = subprocess.Popen(command.split(), stdin=subprocess.PIPE,
                                        stdout=subprocess.PIPE)
...
def standard_run(fasta_input="sample_fasta.fa", output_dir="/home/tb/misc_output/",
                 jellyfish_exe="/home/tb/dpeer_work/jellyfish-2.2.0/bin/jellyfish",
                 k_vals=[60], jelly_num_process=16, jelly_s_str="400M",
                 is_test_limited=False):
    start = time.time()
    ...
    for k in k_vals:
        ...
        gsa.deal_with_patterns(k, output_dir, jellyfish_exe, jelly_num_process,
                               jelly_s_str)
    print("Finished after", time.time() - start, "seconds")


if __name__ == "__main__":
    standard_run(fasta_input="../sa_preprocess/csf_labeled.fa", output_dir="/mnt/jelly/",
                 jellyfish_exe="/root/home/jellyfish-2.2.0/bin/jellyfish", k_vals=[60])
    print("Done all work.")
