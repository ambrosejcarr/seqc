...
class Transcript(Record):
    ...
    def __repr__(self):
        return '<Transcript (contains %d exons and UTRs): %s>' % (len(self), str(self))
...
class Gene(Record):
    ...
    def __repr__(self):
        return '<Gene (contains %d exons and UTRs): %s>' % (len(self), str(self))
    ...
    def intervals(self, start=None, stop=None):
        ...
        return self._merge_intervals(ivs)
